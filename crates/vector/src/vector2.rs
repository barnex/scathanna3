	...
	T: Copy,
{
	...
	/// Append a 3th element to the vector.
	/// ```
	/// # use vector::*;
	/// assert_eq!(ivec2(1,2).append(3), ivec3(1,2,3));
	/// ```
	pub fn append(&self, z: T) -> Vector3<T> {
	...
	T: Div<T, Output = T> + Copy,
{
	...
	/// ```
	/// # use vector::*;
	///	assert_eq!(ivec2(3, 8) / ivec2(3, 2), ivec2(1, 4));
	/// ```
	#[inline]
	...
	T: Mul<T, Output = T> + Copy,
{
	...
	/// ```
	/// # use vector::*;
	///	assert_eq!(ivec2(2, 3) * ivec2(4, 5), ivec2(8, 15));
	/// ```
	#[inline]
...
{
	/// Sum items produced by iterator, starting from zero (default).
	/// ```
	/// # use vector::*;
	/// assert_eq!(Vec::<ivec2>::new().iter().copied().sum::<ivec2>(), ivec2(0,0));
	/// assert_eq!([ivec2(1,2), ivec2(3,4)].iter().copied().sum::<ivec2>(), ivec2(4,6));
	///
	/// ```
	fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
		iter.fold(Vector2::<T>::default(), |sum, term| sum + term)
	}
}
