	...
	T: Copy,
{
	...
	/// Append a 4th element to the vector.
	/// ```
	/// # use vector::*;
	/// assert_eq!(ivec3(1,2,3).append(4), ivec4(1,2,3,4));
	/// ```
	pub fn append(&self, w: T) -> Vector4<T> {
	...
	T: Div<T, Output = T> + Copy,
{
	...
	/// ```
	/// # use vector::*;
	///	assert_eq!(ivec3(3, 8, 10) / ivec3(3, 2, 1), ivec3(1, 4, 10));
	/// ```
	#[inline]
	...
	T: Mul<T, Output = T> + Copy,
{
	...
	/// ```
	/// # use vector::*;
	///	assert_eq!(ivec3(1,2,3) * ivec3(3,4,5), ivec3(3,8,15));
	/// ```
	#[inline]
	...
	T: Default + Add<Output = T> + Copy,
{
	fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
		iter.fold(Self::default(), |sum, term| sum + term)
	}
}
