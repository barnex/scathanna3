...
pub fn integrate<F>(num_samples: Range<u32>, max_err: f64, f: &mut F) -> vec3
where
	F: FnMut() -> vec3,
{
...
fn srgb_intensity(c: vec3) -> f64 {
	f32::sqrt((c.x() + c.y() + c.z()) / 3.0) as f64
}
...
mod test {
	...
	fn integrate_const() {
		...
		assert_eq!(integrate(1..1, 0.0, &mut sample), vec3::new(0.1, 0.2, 0.3));
	}

	#[test]
	fn integrate_noise() {
		let mut rng = rand::thread_rng();
		let mut sample = || vec3::new(rng.gen(), rng.gen(), rng.gen());

		let have: vec3 = integrate(1000..1000, 0.0, &mut sample).into();
		let want: vec3 = vec3::new(0.5, 0.5, 0.5).into();
