...
/// assert_eq!(bb.vertices().collect::<Vec<_>>, vec![]);
///
/// ```
impl<T> BoundingBox<T>
where
	T: Copy,
{
	pub fn vertices(&self) -> impl Iterator<Item = Vector3<T>> {
		let min = self.min;
		let max = self.max;
		[
	...
	T: Float,
{
	...
	pub fn center_bottom(&self) -> Vector3<T> {
	...
	T: Number,
{
	...
	/// assert_eq!(bb, None);
	/// ```
	pub fn from_points<'i, V: ToOwned<Owned = Vector3<T>>>(mut vertices: impl Iterator<Item = V> + 'i) -> Option<Self> {
	...
	pub fn contains_excl(&self, point: Vector3<T>) -> bool {
		point.x() > self.min.x() //.
		&& point.x() < self.max.x()
		&& point.y() > self.min.y()
		&& point.y() < self.max.y()
