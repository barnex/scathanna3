...
/// Map a point (u,v) from the unit square to the unit disk,
/// preserving uniformity.
///
/// This is used for sampling a disk-shaped light source.
pub fn uniform_disk(uv: vec2) -> vec2 {
	let (u, v) = uv.into();
	let theta = (2.0 * PI) * u;
	let r = f32::sqrt(v);
...
pub fn disk_with_normal(uv: vec2, normal: vec3) -> vec3{
	make_basis(normal) * uniform_disk(uv).append(0.0)
}
...
pub fn cosine_sphere(uv: vec2, normal: vec3) -> vec3 {
