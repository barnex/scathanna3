...
pub(crate) struct AnimationPipeline {
...
impl AnimationPipeline {
	pub fn new(opts: &GraphicsOpts, device: &wgpu::Device, surface_format: wgpu::TextureFormat, global_uniforms_layout: &wgpu::BindGroupLayout) -> Self {
		let src = concat!(include_str!("globals.wgsl"), include_str!("animation.wgsl"));
		...
		let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
			...
			vertex: wgpu::VertexState {
				module: &shader,
				entry_point: "vs_main",
				buffers: &[VertexKF::desc()], // <- arguments passed to vs_main (animation.wgsl)
			},
