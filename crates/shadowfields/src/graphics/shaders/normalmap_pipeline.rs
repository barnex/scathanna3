...
/// A shader
pub(crate) struct NormalmapPipeline {
...
impl NormalmapPipeline {
	pub fn new(opts: &GraphicsOpts, device: &wgpu::Device, surface_format: wgpu::TextureFormat, global_uniforms_layout: &wgpu::BindGroupLayout) -> Self {
		let src = concat!(include_str!("globals.wgsl"), include_str!("normalmap.wgsl"));
	...
	pub fn texture_bind_group(&self, device: &Device, texture: &Texture, lightmap: &Texture, normalmap: &Texture, direct: &Texture) -> wgpu::BindGroup {
